...
//             sessionStorage.setItem("user", JSON.stringify(user)); // Phải lưu sessionStorage vì App.jsx đọc ở đó
//             window.dispatchEvent(new Event("loginSuccess")); // Gửi sự kiện để bật chatbot và cập nhật userId
//             navigate("/");
//         } catch (error) {
...
const SignIn = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            localStorage.setItem("token", token);
            sessionStorage.setItem("user", JSON.stringify(user)); // Lưu user để app lấy
            window.dispatchEvent(new Event("loginSuccess")); // Thông báo đăng nhập thành công
            navigate("/");
        } catch (error) {
            alert("Email hoặc mật khẩu không đúng!");
        } finally {
                ...
                <div className="col-md-6 px-5">
                    <div className="mx-auto" style={{ maxWidth: "400px" }}>
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="form-check mb-3">
                                <input className="form-check-input" type="checkbox" id="terms" disabled={loading} />
                                <label className="form-check-label" htmlFor="terms">
                                    I agree the <Link to="#">Terms and Conditions</Link>
                                </label>
                            </div>
                        ...
                        <div className="mt-4">
                            <button className="btn btn-light border w-100 py-2 d-flex align-items-center justify-content-center" disabled={loading}>
                                <img
                                    src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg"
                                    alt="Google"
                                    style={{ width: "20px", marginRight: "10px" }}
